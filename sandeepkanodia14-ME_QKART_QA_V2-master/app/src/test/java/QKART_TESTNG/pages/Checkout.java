...
public class Checkout {
    ...
    public Boolean addNewAddress(String addresString) {
        try {
            ...
            List<WebElement> buttons = driver.findElements(By.xpath("//*[contains(@class,'MuiStack-root css-1wrao5i-MuiStack-root')]//button"));
            for (WebElement button : buttons) {
                if (button.getText().equals("ADD")) {
                    button.click();
                    WebDriverWait wait = new WebDriverWait(driver, 30);
                    wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath(String.format(
                            "//input[@name='address']//..//..//p[text()='%s']",
                            addresString))));
                    Thread.sleep(2000);
                    return true;
                }
    ...
    public Boolean selectAddress(String addressToSelect) {
        try {
            ...
            WebElement address = driver.findElementByXPath(
                    String.format("//p[@class = 'MuiTypography-root MuiTypography-body1 css-uckjdc-MuiTypography-root' and text()= '%s']//preceding-sibling::span//input",
                            addressToSelect));
