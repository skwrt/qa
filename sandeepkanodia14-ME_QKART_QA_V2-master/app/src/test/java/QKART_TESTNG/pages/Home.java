...
public class Home {
    ...
    public Boolean searchForProduct(String product) {
        try {
            ...
            wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath(String
                    .format("//div[contains(@class,'MuiCardContent-root')]/p[contains(text(),'%s')]", product))));
            Thread.sleep(3000);
            return true;
        } catch (Exception e) {
    ...
    public List<WebElement> getSearchResults() {
        ...
        try {
            ...
            // search results
            searchResults = driver.findElementsByClassName("MuiCardContent-root");
    ...
    public Boolean addProductToCart(String productName) {
        try {
             ...
             */
            List<WebElement> gridContent = driver.findElementsByXPath("//*[contains(@class,'MuiGrid-root')]");
            for (WebElement cell : gridContent) {
                if (cell.findElement(By.xpath("//*[contains(@class,'MuiCardContent-root')]")).getText().contains(productName)) {
                    cell.findElement(By.xpath("//*[contains(@class,'MuiCardContent-root')]//following-sibling::div//button")).click();
                    WebDriverWait wait = new WebDriverWait(driver, 30); 
                    wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(
                            String.format("//*[contains(@class,'cart')]//div[contains(text(),'%s')]", productName))));
                    return true;
                }
            }
    ...
    public Boolean verifyCartContents(List<String> expectedCartContents) {
        try {
            List<WebElement> cartContents = driver.findElementsByXPath(
                    "//div[contains(@class,'cart')]//div[contains(@class,'MuiBox-root css-1gjj37g')]/div");
